...
This module provides a backend-agnostic distribution shift attack for regression tasks.
Analogous to label-flip attacks on classification, this attack systematically modifies
continuous target values to degrade model performance through noise injection.
...
class DistributionShiftAttack:
    ...
    def poison_dataset(self, X: np.ndarray, y: np.ndarray) -> Tuple:
        """
        Poison features and targets of a dataset.

        In practice: X is unchanged, only y is modified. Features remain
        indistinguishable from clean data—making this attack stealthy.
    ...
    def poison_dataloader(
        ...
        Each batch's targets are corrupted according to the attack parameters.
